--[[
	...
	Revision: $Id: CoreScan.lua 6000 2018-07-17 14:09:34Z none $
...
do
	...
	local function GetItemInfoCache2(link, itemID, scanthrottle)
		if scanthrottle and ItemTried and ItemTried[itemID] then
			-- if GetItemInfo previously failed for a link with this cachekey in this processing pass (ItemTried is reset each pass)
			return
		end
		local _,_,_,iLevel,uLevel,_,_,_,equipLoc,_,_,classID,subClassID = AucAdvanced.GetItemInfoCache(link)
		if not classID then
			if scanthrottle then
				ItemTried[itemID] = true
			end
	...
	function private.GetAuctionItemFillIn(itemData, scanthrottle) -- ### Legion : revised, check
		...
		if itemID == 82800 then -- "Pet Cage"
			...
			if not itemData[Const.CLASSID] then
				...
				local classID, subClassID, equipCode, iLevel, uLevel = GetItemInfoCache2(itemLink, itemID, scanthrottle) -- {iType, iSubtype, Const.EquipEncode[equipLoc], iLevel, uLevel}
				if classID then
...
AucAdvanced.RegisterRevision("$URL: /CoreScan.lua $", "$Rev: 6000 $")

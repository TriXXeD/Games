...
function addon.LfgScan(group)
    ...
    C_LFGList.Search(addon.lfgGroups[curLfgGroup],LFGListSearchPanel_ParseSearchTerms(""),nil,nil,languages)
end
...
local function CreateHopRaid()
    if canJoinGroup() then
        C_LFGList.CreateListing(16,"Realm Hop Assist-"..homeRealm,0,0,"","Realm Hop Assist - Raid (Farming) - "..homeRealm.." - "..zoneName,true,false)
...
function addon.toggleMinimapIcon(hidden)
    if hidden then
        DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Type |cffffffff/cra |cffff0000or |cffffffff/rha |cffff0000in chat to open Realm Hop Assist without minimap button");
...
local function initMenu(self, level)
    ...
    if level == 1 then
		...
		if canJoinGroup() then
            ...
            UIDropDownMenu_AddButton({text="Create a raid for realm hoppers",notCheckable=1,func=CreateHopRaid}, level)

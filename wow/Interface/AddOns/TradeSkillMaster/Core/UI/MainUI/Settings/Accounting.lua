...
function private.GetAccountingSettingsFrame()
	...
	return TSMAPI_FOUR.UI.NewElement("ScrollFrame", "accountingSettings")
		...
		:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "row1Dropdowns")
			:SetLayout("HORIZONTAL")
			:SetStyle("height", 26)
			:SetStyle("margin.bottom", 16)
			:AddChild(TSMAPI_FOUR.UI.NewElement("SelectionDropdown", "timeFormatDropdown")
				...
				:SetSettingInfo(TSM.db.global.accountingOptions, "timeFormat")
			)
			:AddChild(TSMAPI_FOUR.UI.NewElement("SelectionDropdown", "timeFormatDropdown")
				:SetItems(private.marketValueItems, private.marketValueKeys)
				:SetSettingInfo(TSM.db.global.accountingOptions, "mvSource")
			)
		)

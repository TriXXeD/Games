...
function private.GetSendFrame()
	local frame = TSMAPI_FOUR.UI.NewElement("Frame", "send")
		...
		:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "container")
			...
			:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "dragBox")
				:SetLayout("HORIZONTAL")
				...
				:AddChild(TSMAPI_FOUR.UI.NewElement("QueryScrollingTable", "items")
					:SetStyle("anchors", { { "TOPLEFT" }, { "BOTTOMRIGHT", 0, 26 } })
					:SetStyle("headerBackground", "#00404040")
					:SetStyle("background", "#1b1b1b")
		...
		:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "footer")
			...
			:AddChild(TSMAPI_FOUR.UI.NewElement("ActionButton", "sendMail")
				...
				:SetText(L["SEND MAIL"])
				:SetScript("OnClick", private.SendMailOnClick)
				:SetDisabled(true)
			)
...
function private.SubjectBtnOnClick(button)
	local frame = TSMAPI_FOUR.UI.NewElement("Frame", "frame")
		...
		:AddChild(TSMAPI_FOUR.UI.NewElement("Input", "descriptionInput")
		...
		:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "footer")
			...
			:AddChild(TSMAPI_FOUR.UI.NewElement("Text", "title")
				...
				:SetStyle("fontHeight", 10)
				:SetStyle("justifyH", "RIGHT")
				:SetFormattedText(L["(%d/500 Characters)"], 0)
			)
...
function private.DragBoxOnItemRecieve(frame, button)
	if not CursorHasItem() or private.query:Count() >= 12 then
		ClearCursor()
		return
	end
...
function private.SubjectClearAllBtnOnClick(button)
	...
	button:GetParentElement():GetParentElement():GetElement("subjectInput")
		:SetFocused(false)
		:SetText(private.subject)
		:Draw()
	button:GetParentElement():GetParentElement():GetElement("descriptionInput")
		:SetFocused(false)
		:SetText(private.body)
		:Draw()
...
function private.DesciptionOnTextChanged(input)
	...
	input:GetElement("__parent.footer.title"):SetFormattedText(L["(%d/500 Characters)"], strlen(private.body))
		:Draw()
end
...
function private.MoneyOnTextChanged(input)
	local text =  gsub(strtrim(input:GetText()), LARGE_NUMBER_SEPERATOR, "")
...
function private.SendMailOnClick(button)
	if private.frame:GetElement("container.check.sendCheck"):IsChecked() and private.money >= 10000000 then
		button:GetBaseElement():ShowConfirmationDialog(L["Send Mail Confirmation"], L["Recipient"]..": "..private.recipient.."\n"..L["Send Money"]..": "..TSMAPI_FOUR.Money.ToString(private.money, "OPT_PAD", "OPT_SEP"), strupper(YES), private.SendMail)
	else
...
function private.SendMail(button)
	...
	if private.query:Count() > 0 then
		...
		private.fsm:ProcessEvent("EV_BUTTON_CLICKED", private.recipient, private.subject, private.body, money, items)
	else
		private.fsm:ProcessEvent("EV_BUTTON_CLICKED", private.recipient, private.subject, private.body, money)
	end
end
...
function private.FSMCreate()
	...
	local function ClearMail(context, redraw)
		private.recipient = ""
		private.subject = ""
		private.body = ""
		private.money = 0
	...
	private.fsm = TSMAPI_FOUR.FSM.New("MAILING_SEND")
		...
		:AddState(TSMAPI_FOUR.FSM.NewState("ST_SENDING_START")
			:SetOnEnter(function(context, recipient, subject, body, money, items)

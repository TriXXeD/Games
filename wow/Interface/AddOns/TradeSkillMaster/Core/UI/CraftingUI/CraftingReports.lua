			...
			:AddChild(TSMAPI_FOUR.UI.NewElement("QueryScrollingTable", "mats")
				...
				:GetScrollingTableInfo()
					...
					:NewColumn("price")
						...
						:SetFontHeight(12)
						:SetJustifyH("RIGHT")
						:SetTextInfo("itemString", private.MatsGetPriceText)
						:SetSortInfo("spellId") -- TODO
						:Commit()
...
function private.CraftsGetCostItemValueText(costItemValue)
	if costItemValue == math.huge then
		return ""
	end
...
function private.CraftsGetSaleRateText(saleRate)
	if saleRate == math.huge then
		return ""
	end
	return format("%0.2f", saleRate)
end

function private.MatsGetPriceText(itemString)
	return TSMAPI_FOUR.Money.ToString(TSM.Crafting.Cost.GetMatCost(itemString), "OPT_PAD")
end

...
function Shopping.StartGatheringSearch_DEPRECATED(itemString, need, buyCallback)
...
function private.PostDialogShow(baseFrame, record)
	...
	local frame = TSMAPI_FOUR.UI.NewElement("Frame", "frame")
		...
		:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "quantity")
			...
			:AddChild(TSMAPI_FOUR.UI.NewElement("Text", "of")
				:SetStyle("font", TSM.UI.Fonts.RobotoMedium)
				:SetStyle("fontHeight", 12)
				:SetStyle("justifyH", "RIGHT")
				:SetStyle("margin", { left = -22 })
				:SetText(L["of"])
			)
			...
			:AddChild(TSMAPI_FOUR.UI.NewElement("Button", "editBtn")
				...
				:SetScript("OnClick", private.StackQuantityEditBtnOnClick)
			)
		)
...
function private.StartFilterSearchHelper(viewContainer, filter, pctSource, buyCallback, isSpecial)
	...
	private.fsm:ProcessEvent("EV_START_SCAN", threadId, marketValueFunc, buyCallback or NoOp, NoOp, filter, filter)
end
...
function private.StackQuantityEditBtnOnClick(button)
...
function private.StackBtnOnClick(button)
	...
	if private.perItem then
		...
		private.perItem = true
		private.BuyoutTextOnValueChanged(buyoutText, TSMAPI_FOUR.Money.ToString(TSMAPI_FOUR.Util.Floor(buyout / stackSize) - undercut))
		private.BidTextOnValueChanged(bidText, TSMAPI_FOUR.Money.ToString(TSMAPI_FOUR.Util.Floor(bid / stackSize) - undercut))

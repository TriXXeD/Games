...
function private.GroupsMailThread(groupList, sendRepeat)
	while true do
		...
		if sendRepeat then
			TSMAPI_FOUR.Thread.Sleep(TSM.db.global.mailingOptions.resendDelay)
		else
			break
		end
	end
...
function private.SendItems(groupPath, operationSettings)
	...
	if next(items) then
		...
		TSM.Mailing.Send.StartSending(private.SendCallback, operationSettings.target, "", "", 0, items)
...
function private.GetTargetQuantity(player, itemString, sources)
	...
	if sources then
		if sources.guild then
			num = num + TSMAPI_FOUR.Inventory.GetGuildQuantity(itemString, TSMAPI_FOUR.Player.GetPlayerGuild(player))
		end

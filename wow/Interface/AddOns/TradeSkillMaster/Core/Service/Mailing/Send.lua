...
function Send.StartSending(callback, recipient, subject, body, money, items)
	TSMAPI_FOUR.Thread.Kill(private.thread)

	TSMAPI_FOUR.Thread.SetCallback(private.thread, callback)
	TSMAPI_FOUR.Thread.Start(private.thread, recipient, subject, body, money, items)
end
...
function private.SendMailThread(recipient, subject, body, money, items)
	...
	for _, bag, slot, itemString, quantity in TSMAPI_FOUR.Inventory.BagIterator(true, false, true) do
...
function private.SendMail(recipient, subject, body, money)
	...
	-- TODO: sendDelay could also be 0 here
	--TSMAPI_FOUR.Thread.Sleep(TSM.db.global.mailingOptions.sendDelay)

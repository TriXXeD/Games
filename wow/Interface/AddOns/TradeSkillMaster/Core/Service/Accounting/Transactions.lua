...
function Transactions.GetBuyStats(itemString)
	...
	if private.buyStatsCache.lastUpdate ~= GetTime() then
		...
		wipe(private.buyStatsCache.totalPrice)
		private.buyStatsCache.lastUpdate = GetTime()
	end

	-- check if we have the values cached
	if private.buyStatsCache.totalPrice[itemString] then
		return private.buyStatsCache.totalPrice[itemString], private.buyStatsCache.totalNum[itemString]
	end
	...
	if remainingNum then
		...
		totalNum = private.statsQuery:Sum("quantity")
		if not totalNum then
			return
		end
		totalPrice = private.statsQuery:SumOfProduct("quantity", "price")
	end
	private.buyStatsCache.totalPrice[itemString] = totalPrice

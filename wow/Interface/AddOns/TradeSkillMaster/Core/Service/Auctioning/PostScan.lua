...
function private.GetPostBagSlot(itemString, quantity)
	...
	assert(bag and slot)
	bag, slot = private.ItemBagSlotHelper(itemString, bag, slot, quantity)
...
function private.ItemBagSlotHelper(itemString, bag, slot, quantity)
	...
	local found = private.bagDB:NewQuery()
		...
		:GetFirstResultAndRelease()
	if found then
		private.RemoveBagQuantity(slotId, quantity)
		return bag, slot
	end
	...
	local foundSlotId, foundBag, foundSlot = private.bagDB:NewQuery()
		...
		:GetFirstResultAndRelease()
	if foundSlotId then
		private.RemoveBagQuantity(foundSlotId, quantity)
	...
	query:Release()
	if numNeeded == numUsed then
		private.RemoveBagQuantity(slotId, selectedQuantity)
		for _, rowSlotId in TSMAPI_FOUR.Util.TempTableIterator(usedSlotIds) do
			...
			private.RemoveBagQuantity(rowSlotId, rowNumUsed)
		end
		return bag, slot
	else
	...
	local rowBag, rowSlot = private.bagDB:NewQuery()
		...
		:OrderBy("slotId", true)
		:GetFirstResultAndRelease()
	return private.ItemBagSlotHelper(itemString, rowBag, rowSlot, quantity)
end

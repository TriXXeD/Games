...
do
	...
	local function HasValidSteps(text)
		...
		if textLines[1] == STEPS_LINES[1] then
			...
			return false
		elseif strmatch(textLines[1], "^"..STEPS_LINES[1]) and #textLines == 1 and #textLines[1] - #STEPS_LINES[1] < 20 then
			-- they just added a minimal amount of text to the end of the first line
			return false
		end
		...
		for _, line in ipairs(textLines) do
			...
			if select("#", strsplit(" ", line)) >= 2 and #line >= 10 and numChars > 6 then
				return true
			end
		end
		return false

...
function MailTracking.OnInitialize()
	...
	private.db = TSMAPI_FOUR.Database.New(INBOX_DB_SCHEMA, "MAILTRACKING_INBOX_INFO")
...
function private.MailInboxUpdateDelayed()
	...
	for i = 1, GetInboxNumItems() do
		...
		local firstItemString = TSMAPI_FOUR.Item.ToItemString(private.GetFirstInboxItemLink(i)) or ""
		local item = private.GetFirstInboxItemLink(i) or ""
		local _, _, _, firstQuality = GetInboxItem(i, 1)
		...
		private.db:BulkInsertNewRow(i, mailType, subject or "--", firstItemString, item, firstQuality or 0, itemCount or 0, money or 0, cod or 0, daysLeft)
...
function private.GetFirstInboxItemLink(index)
	...
	local _, speciesId, level, breedQuality, maxHealth, power, speed = private.tooltip:SetInboxItem(index)
	local link
